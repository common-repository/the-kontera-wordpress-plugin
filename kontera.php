...
function kontera_is_page_enabled() {
...
function kontera_echo_javascript() {
	$toggle_kontera_tag = kontera_is_ad_enabled();
...
?>
	...
	echo "<tr><td> $parameters[kontera_disabled_pages] </td> <td> <textarea name='kontera_disabled_pages' >" . get_option('kontera_disabled_pages') . "</textarea> </td></tr>";
?>
...
function kontera_add_zone_tag_to_content($text) {
	...
	$kontera_ads_disabled_on_page = (kontera_is_young_post() || !kontera_should_show() || !kontera_is_page_enabled());
...
function kontera_write_post_or_page() {
    if($_POST) {
...
add_action('edit_post', 'kontera_write_post_or_page');
add_action('edit_page', 'kontera_write_post_or_page');
